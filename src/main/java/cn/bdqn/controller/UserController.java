...
import cn.bdqn.service.dataDictionary.DataDictionaryService;
...
public class UserController extends BaseController{
	...
	@RequestMapping("/backend/modifyPwd")
	@ResponseBody
	public Object modifyPwd(HttpSession session,@RequestParam String newpwd,@RequestParam String oldpwd){
		User sessionUser =  ((User)session.getAttribute(Constants.SESSION_USER));
		try {
		   if(oldpwd.equals(sessionUser.getPassword())){
			   sessionUser.setPassword(newpwd);
			  int result=userService.modifyUser(sessionUser);
			  if(result<0){
				  return "failed";
			  }
		   }else{
	...
	@RequestMapping("/member/savesecondpwd")
	@ResponseBody
	public Object saveSecondPwd(HttpSession session,@RequestParam String newpwd,@RequestParam String oldpwd){
		User sessionUser =  ((User)session.getAttribute(Constants.SESSION_USER));
		try {
			if(oldpwd.equals(sessionUser.getPassword())){
				sessionUser.setPassword(newpwd);
	...
	public ModelAndView savePersonalInfo(HttpSession session,@ModelAttribute("modifyUser") User modifyUser){
		if(session.getAttribute(Constants.SESSION_BASE_MODEL) == null){
			...
			try {
				modifyUser.setLastUpdateTime(new Date());
				userService.modifyUser(modifyUser);
				session.setAttribute(Constants.SESSION_USER,userService.getUserById(modifyUser.getId()));
	...
	@RequestMapping(value = "/backend/loadUserTypeList")
	@ResponseBody
	public Object loadUserTypeList(@RequestParam(value="s_roleId",required=false) String s_roleId){
		...
		try {
			...
			cjson = JSON.toJSONString(userTypeList);
		} catch (Exception e) {
			e.printStackTrace();
		}
			return cjson;
    ...
    public Object upload(@RequestParam(value = "a_fileInputID", required = false) MultipartFile cardFile,
        ...
        if(cardFile != null){
            ...
            if(cardFile.getSize() >  filesize){//上传大小不得超过 50k
            	...
            	String fileName = System.currentTimeMillis()+new Random().nextInt(1000000)+"_IDcard.jpg";
        ...
        if(bankFile != null){
            ...
            if(bankFile.getSize() >  filesize){//上传大小不得超过 50k
            	...
            	String fileName = System.currentTimeMillis()+new Random().nextInt(1000000)+"_bank.jpg";
	...
	@RequestMapping(value = "/backend/delpic")
	@ResponseBody
	...
	@RequestMapping(value = "/backend/deluser")
	@ResponseBody
		...
		try {
			...
			if(delUserType.equals("2") || delUserType.equals("3") || delUserType.equals("4")){
				...
				if(this.delPic(delIdCardPicPath,delId,request,session).equals("success") && this.delPic(delBankPicPath,delId,request,session).equals("success")){
					if(userService.deleteUser(Integer.parseInt(delId)) > 0)
						result = "success";
				}
			}
		} catch (Exception e) {
	...
	@RequestMapping(value = "/logincodeisexit")
	@ResponseBody
	public boolean loginCodeIsExit(@RequestParam(value="loginCode",required=false) String loginCode){
		logger.debug(" loginCodeIsExit loginCode===================== "+loginCode);
       try{
           int result=  userService.loginCodeIsExits(loginCode);
           if(result>0){
               return  true;
           }
       }catch (Exception e){
	...
	@RequestMapping(value = "/backend/getuser")
	@ResponseBody
	...
	public ModelAndView registOk(HttpSession session,Model model,
		...
		if(baseModel == null){
			...
			User _user = new User();
			try {
				_user = userService.getUserList(registUser).get(0);
