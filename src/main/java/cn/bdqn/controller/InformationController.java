...
import cn.bdqn.service.uploadtemp.UploadTempService;
import cn.bdqn.util.Constants;
import cn.bdqn.util.HtmlEncode;
		...
		if(null == id || "".equals(id)){
			...
			try {
				...
				if(null != information && information.getTitle() != null){
					...
					result =  JSONObject.toJSONString(information, SerializerFeature.WriteDateUseDateFormat);
				}
			} catch (Exception e) {
				result =  "failed";
			}
...
//			logger.debug("+++++++++++++++++++++++++++++:" + afficheList.size());
	...
	public Object modifyRole(HttpSession session,@RequestParam String inforState){
		...
		if(null == inforState || "".equals(inforState)){
			...
			JSONObject informationObject = JSONObject.parseObject(inforState);
			Information information =  (Information)JSONObject.toJavaObject(informationObject, Information.class);
			information.setUploadTime(new Date());
